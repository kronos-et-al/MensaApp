...
use crate::{util::Uuid, interface::api_command::Command, layer::logic::api_command::auth};
...
impl MutationRoot {
    ...
    async fn add_image(
        ...
        command.add_image(meal_id, image_url, auth_info).await.map_err(|e| e.to_string())?;
        Ok(true)
    }
}
