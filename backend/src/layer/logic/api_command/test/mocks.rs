...
impl CommandDataAccess for CommandDatabaseMock {
    ...
    async fn add_upvote(&self, _image_id: Uuid, _user_id: Uuid) -> DataResult<()> {
        Ok(())
    }

    /// Adds a downvote to the given image. An user can only down- or upvote an image.
    async fn add_downvote(&self, _image_id: Uuid, _user_id: Uuid) -> DataResult<()> {
        Ok(())
    }

    /// Removes an upvote from the given image.
    async fn remove_upvote(&self, _image_id: Uuid, _user_id: Uuid) -> DataResult<()> {
        Ok(())
    }

    /// Removes a downvote from the given image.
    async fn remove_downvote(&self, _image_id: Uuid, _user_id: Uuid) -> DataResult<()> {
        Ok(())
    }
    ...
    async fn add_rating(&self, _meal_id: Uuid, _user_id: Uuid, _rating: u32) -> DataResult<()> {
        Ok(())
    }
