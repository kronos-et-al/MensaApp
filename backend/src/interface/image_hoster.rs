...
pub trait ImageHoster {
    ...
    async fn validate_url(&self, url: String) -> Result<ImageMetaData>;
    /// Checks if an image still exists at the hoster website.
    async fn check_existence(&self, photo_id: String) -> Result<bool>;
    /// Checks whether the licence is acceptable for our purposes.
    async fn check_licence(&self, photo_id: String) -> Result<bool>;
}
