...
pub trait Command {
    ...
    async fn report_image(image_id: Uuid, reason: ReportReason, auth_info: AuthInfo) -> Result<()>;

    /// Command to vote up an image. All down-votes of the same user get removed.
    async fn add_image_upvote(image_id: Uuid, auth_info: AuthInfo) -> Result<()>;

    /// Command to vote down an image. All up-votes of the same user get removed.
    async fn add_image_downvote(image_id: Uuid, auth_info: AuthInfo) -> Result<()>;

    /// Command to remove an up-vote for an image.
    async fn remove_image_upvote(image_id: Uuid, auth_info: AuthInfo) -> Result<()>;

    /// Command to remove an down-vote for an image.
    async fn remove_image_downvote(image_id: Uuid, auth_info: AuthInfo) -> Result<()>;

    /// Command to link an image to a meal.
    async fn add_image(meal_id: Uuid, image_url: String, auth_info: AuthInfo) -> Result<()>;

    /// command to add a rating to a meal.
    async fn set_meal_rating(meal_id: Uuid, rating: u32, auth_info: AuthInfo) -> Result<()>;
}
