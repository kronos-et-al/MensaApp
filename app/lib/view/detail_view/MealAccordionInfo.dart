      ...
      children: [
        ...
        if (_allergens.isNotEmpty)
          Padding(
            ...
            child: Text(
              "${FlutterI18n.translate(context, _allergens.isNotEmpty ? "allergen.allergenTitle" : "allergen.allergenTitleEmpty")} ${_allergens.length > 1 ? ([_allergens.getRange(0, _allergens.length - 1).map((e) => FlutterI18n.translate(context, "allergen.${e.name}")).join(", "), FlutterI18n.translate(context, "allergen.${_allergens[_allergens.length - 1].name}")].join(FlutterI18n.translate(context, "allergen.allergenSeparator"))) : FlutterI18n.translate(context, "allergen.${_allergens[0].name}")}",
              style: const TextStyle(fontWeight: FontWeight.w300),
            ),
          ),
        const SizedBox(height: 8),
