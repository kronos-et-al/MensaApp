...
class SQLiteDatabaseAccess implements IDatabaseAccess {
      ...
      onUpgrade: (db, oldVersion, newVersion) {
        for (var i = oldVersion; i < newVersion; i++) {
          if (dbMigrations.containsKey((i, i + 1))) {
            Future.forEach(dbMigrations[(i, i + 1)]!, (sql) async => await db.execute(sql));
          }
        }
      },
      version: 3,
  ...
  @override
  Future<int> addFavorite(Meal meal, DateTime servedDate, Line servedLine) async {
  ...
  Future<int> deleteFavorite(Meal meal) async {
    var db = await database;
    return db.delete(DBFavorite.tableName, where: '${DBFavorite.columnMealID} = ?', whereArgs: [meal.id]);
  }
    ...
    for (Map<String, dynamic> favoriteMap in dbFavoritesListResult) {
      ...
      if (dbMeal != null) {
        Meal meal = DatabaseTransformer.fromDBMeal(
            dbMeal,
            Price(student: dbFavorite.priceStudent, employee: dbFavorite.priceEmployee, pupil: dbFavorite.priceEmployee, guest: dbFavorite.priceGuest),
  ...
  Future<Result<Meal, NoMealException>> getMealFavorite(String id) async {
    var db = await database;
    var result = await db.query(DBFavorite.tableName, where: '${DBFavorite.columnMealID} = ?', whereArgs: [id]);
  ...
  @override
  Future<Result<List<MealPlan>, NoDataException>> getMealPlan(DateTime date, Canteen canteen) async {
    var db = await database;
    var result = await db.query('${DBMealPlan.tableName}, ${DBCanteen.tableName}, ${DBLine.tableName}',
        where:
    ...
    if (result.isNotEmpty) {
      ...
      for (DBMealPlan dbMealPlan in result.map((plan) => DBMealPlan.fromMap(plan))) {
        ...
        mealPlans.add(DatabaseTransformer.fromDBMealPlan(dbMealPlan, dbLine, dbCanteen!, meals));
      }
  ...
  Future<List<Meal>> _getMealPlanMeals(DBMealPlan dbMealPlan) async {
    List<DBMealPlanMeal> dbMealPlanMeals = await _getDBMealPlanMeals(dbMealPlan.mealPlanID);
    ...
    for (DBMealPlanMeal dbMealPlanMeal in dbMealPlanMeals) {
      ...
      Meal meal = Meal(
          ...
          price: Price(student: dbMealPlanMeal.priceStudent, employee: dbMealPlanMeal.priceEmployee, pupil: dbMealPlanMeal.pricePupil, guest: dbMealPlanMeal.priceGuest),
  ...
  Future<List<Side>> _getMealPlanSides(DBMealPlan dbMealPlan, DBMealPlanMeal dbMealPlanMeal) async {
    List<DBMealPlanSide> dbMealPlanSides = await _getDBMealPlanSides(dbMealPlan.mealPlanID, dbMealPlanMeal.mealID);
    ...
    for (DBMealPlanSide dbMealPlanSide in dbMealPlanSides) {
      ...
      Side side = Side(
          ...
          price: Price(student: dbMealPlanSide.priceStudent, employee: dbMealPlanSide.priceEmployee, pupil: dbMealPlanSide.pricePupil, guest: dbMealPlanSide.priceGuest),
  ...
  Future<void> updateImage(ImageData image) async {
    ...
    if (dbImage != null) {
      dbImage = DBImage(image.id, dbImage.mealID, image.url, image.imageRank, image.positiveRating, image.negativeRating, image.individualRating);
      await db.update(DBImage.tableName, dbImage.toMap(), where: '${DBImage.columnImageID} = ?', whereArgs: [image.id]);
    }
  }
  ...
  Future<int> _insertLine(Line line) async {
    ...
    return db.insert(DBLine.tableName, dbLine.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertCanteen(Canteen canteen) async {
    var db = await database;
    var dbCanteen = DBCanteen(canteen.id, canteen.name);
    return db.insert(DBCanteen.tableName, dbCanteen.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertMealPlan(MealPlan mealPlan) async {
    var db = await database;
    var result = await db
        .query(DBMealPlan.tableName, where: '${DBMealPlan.columnLineID} = ? AND ${DBMealPlan.columnDate} = ?', whereArgs: [mealPlan.line.id, _dateFormat.format(mealPlan.date)]);
    DBMealPlan? dbMealPlan;
    if (result.isNotEmpty) {
      DBMealPlan oldDbMealPlan = DBMealPlan.fromMap(result.first);
      dbMealPlan = DBMealPlan(oldDbMealPlan.mealPlanID, mealPlan.line.id, _dateFormat.format(mealPlan.date), mealPlan.isClosed);
    } else {
      var uuid = const Uuid();
      dbMealPlan = DBMealPlan(uuid.v4(), mealPlan.line.id, _dateFormat.format(mealPlan.date), mealPlan.isClosed);
    }
    int id = await db.insert(DBMealPlan.tableName, dbMealPlan.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
    await db.delete(DBMealPlanMeal.tableName, where: '${DBMeal.columnMealPlanID} = ?', whereArgs: [dbMealPlan.mealPlanID]);
    await db.delete(DBMealPlanSide.tableName, where: '${DBMealPlanSide.columnMealPlanID} = ?', whereArgs: [dbMealPlan.mealPlanID]);
  ...
  Future<int> _insertMeal(Meal meal) async {
    ...
    await db.delete(DBMealAllergen.tableName, where: '${DBMealAllergen.columnMealID} = ?', whereArgs: [meal.id]);
    await db.delete(DBMealAdditive.tableName, where: '${DBMealAdditive.columnMealID} = ?', whereArgs: [meal.id]);
    await db.delete(DBImage.tableName, where: '${DBImage.columnMealID} = ?', whereArgs: [meal.id]);
    await db.delete(DBMealNutritionData.tableName, where: '${NutritionDataMixin.columnEntityID} = ?', whereArgs: [meal.id]);

    DBMeal dbMeal = DBMeal(meal.id, meal.name, meal.foodType, meal.individualRating, meal.numberOfRatings, meal.averageRating);

    await Future.wait(meal.allergens?.map((e) => _insertMealAllergen(e, dbMeal)).toList() ?? []);
    await Future.wait(meal.additives?.map((e) => _insertMealAdditive(e, dbMeal)).toList() ?? []);
    await Future.wait(meal.images?.map((e) => _insertImage(e, dbMeal)).toList() ?? []);

    if (meal.nutritionData != null) {
      await _insertMealNutritionData(meal.nutritionData!, dbMeal);
    }

    return await db.insert(DBMeal.tableName, dbMeal.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<int> _insertMealPlanMeal(Meal meal, DBMealPlan mealPlan) async {
    ...
    await Future.wait(meal.allergens?.map((e) => _insertMealAllergen(e, dbMeal)).toList() ?? []);
    await Future.wait(meal.additives?.map((e) => _insertMealAdditive(e, dbMeal)).toList() ?? []);
    await Future.wait(meal.sides!.map((e) => _insertMealPlanSide(e, dbMeal, mealPlan)).toList());
    await Future.wait(meal.images?.map((e) => _insertImage(e, dbMeal)).toList() ?? []);
    if (meal.nutritionData != null) {
      await _insertMealNutritionData(meal.nutritionData!, dbMeal);
    }
    await db.insert(DBMeal.tableName, dbMeal.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
    return await db.insert(DBMealPlanMeal.tableName, mealPlanMeal.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertMealPlanSide(Side side, DBMeal meal, DBMealPlan mealPlan) async {
    var db = await database;
    await db.delete(DBSideAllergen.tableName, where: '${DBSideAllergen.columnSideID} = ?', whereArgs: [side.id]);
    await db.delete(DBSideAdditive.tableName, where: '${DBSideAdditive.columnSideID} = ?', whereArgs: [side.id]);
    await db.delete(DBSideNutritionData.tableName, where: '${NutritionDataMixin.columnEntityID} = ?', whereArgs: [side.id]);
    DBSide dbSide = DBSide(side.id, side.name, side.foodType);
    DBMealPlanSide mealPlanSide = DBMealPlanSide(mealPlan.mealPlanID, meal.mealID, dbSide.sideID, side.price.student, side.price.employee, side.price.pupil, side.price.guest);
    await Future.wait(side.allergens.map((e) => _insertSideAllergen(e, dbSide)));
    await Future.wait(side.additives.map((e) => _insertSideAdditive(e, dbSide)));
    if (side.nutritionData != null) {
      await _insertSideNutritionData(side.nutritionData!, dbSide);
    }
    await db.insert(DBSide.tableName, dbSide.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  Future<int> _insertMealAllergen(Allergen allergen, DBMeal meal) async {
    ...
    return await db.insert(DBMealAllergen.tableName, dbMealAllergen.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertMealAdditive(Additive additive, DBMeal meal) async {
    var db = await database;
    var dbMealAdditive = DBMealAdditive(meal.mealID, additive);
    return await db.insert(DBMealAdditive.tableName, dbMealAdditive.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<int> _insertMealNutritionData(NutritionData nutritionData, DBMeal meal) async {
    ...
    return await db.insert(DBMealNutritionData.tableName, dbMealNutritionData.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertSideAllergen(Allergen allergen, DBSide side) async {
    var db = await database;
    var dbSideAllergen = DBSideAllergen(side.sideID, allergen);
    return await db.insert(DBSideAllergen.tableName, dbSideAllergen.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertSideAdditive(Additive additive, DBSide side) async {
    var db = await database;
    var dbSideAdditive = DBSideAdditive(side.sideID, additive);
    return await db.insert(DBSideAdditive.tableName, dbSideAdditive.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertSideNutritionData(NutritionData nutritionData, DBSide side) async {
    var db = await database;
    ...
    return await db.insert(DBSideNutritionData.tableName, dbSideNutritionData.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _insertImage(ImageData image, DBMeal meal) async {
    var db = await database;
    var dbImage = DBImage(image.id, meal.mealID, image.url, image.imageRank, image.positiveRating, image.negativeRating, image.individualRating);
    return await db.insert(DBImage.tableName, dbImage.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<DBCanteen?> _getDBCanteen(String canteenID) async {
    var db = await database;
    var result = await db.query(DBCanteen.tableName, where: '${DBCanteen.columnCanteenID} = ?', whereArgs: [canteenID]);
    if (result.isNotEmpty) {
  ...
  Future<DBLine?> _getDBLine(String lineID) async {
    var db = await database;
    var result = await db.query(DBLine.tableName, where: '${DBLine.columnLineID} = ?', whereArgs: [lineID]);
    if (result.isNotEmpty) {
  ...
  Future<DBMeal?> _getDBMeal(String mealID) async {
    var db = await database;
    var result = await db.query(DBMeal.tableName, where: '${DBMeal.columnMealID} = ?', whereArgs: [mealID]);
    if (result.isNotEmpty) {
  ...
  Future<DBSide?> _getDBSide(String sideID) async {
    var db = await database;
    var result = await db.query(DBSide.tableName, where: '${DBSide.columnSideID} = ?', whereArgs: [sideID]);
    if (result.isNotEmpty) {
  ...
  Future<List<DBMealPlanMeal>> _getDBMealPlanMeals(String mealPlanID) async {
    var db = await database;
    var result = await db.query(DBMealPlanMeal.tableName, where: '${DBMealPlanMeal.columnMealPlanID} = ?', whereArgs: [mealPlanID]);
    return result.map((mealRow) => DBMealPlanMeal.fromMap(mealRow)).toList();
  }

  Future<List<DBMealPlanSide>> _getDBMealPlanSides(String mealPlanID, String mealID) async {
    var db = await database;
    var result = await db.query(DBMealPlanSide.tableName, where: '${DBMealPlanSide.columnMealPlanID} = ? AND ${DBMealPlanSide.columnMealID} = ?', whereArgs: [mealPlanID, mealID]);
  ...
  Future<DBImage?> _getDBImage(String imageID) async {
    var db = await database;
    var result = await db.query(DBImage.tableName, where: '${DBImage.columnImageID} = ?', whereArgs: [imageID]);
    if (result.isNotEmpty) {
  ...
  Future<List<DBImage>> _getDBImages(String mealID) async {
    var db = await database;
    var result = await db.query(DBImage.tableName, where: '${DBImage.columnMealID} = ?', whereArgs: [mealID]);
    return result.map((imageRow) => DBImage.fromMap(imageRow)).toList();
  }

  Future<DBMealPlan?> _getDBMealPlan(String mealPlanID) async {
    var db = await database;
    var result = await db.query(DBMealPlan.tableName, where: '${DBMealPlan.columnMealPlanID} = ?', whereArgs: [mealPlanID]);
    if (result.isNotEmpty) {
  ...
  Future<DBFavorite?> _getDBFavorite(String favoriteID) async {
    var db = await database;
    var result = await db.query(DBFavorite.tableName, where: '${DBFavorite.columnMealID} = ?', whereArgs: [favoriteID]);
    if (result.isNotEmpty) {
  ...
  Future<List<Map<String, Object?>>> _getDBNutritionDataRaw(String id, NutritionEntity entity) async {
    var db = await database;
    var table = entity == NutritionEntity.meal ? DBMealNutritionData.tableName : DBSideNutritionData.tableName;
    return await db.query(table, where: '${NutritionDataMixin.columnEntityID} = ?', whereArgs: [id]);
  }

  Future<List<Allergen>?> _getMealAllergens(String id) async {
    var db = await database;
    var result = await db.query(DBMealAllergen.tableName, where: '${DBMealAllergen.columnMealID} = ?', whereArgs: [id]);
  ...
  Future<List<Allergen>?> _getSideAllergens(String id) async {
    var db = await database;
    var result = await db.query(DBSideAllergen.tableName, where: '${DBSideAllergen.columnSideID} = ?', whereArgs: [id]);
    return result.map((allergenMap) => DBSideAllergen.fromMap(allergenMap).allergen).toList();
  }

  Future<List<Additive>?> _getMealAdditives(String id) async {
    var db = await database;
    var result = await db.query(DBMealAdditive.tableName, where: '${DBMealAdditive.columnMealID} = ?', whereArgs: [id]);
  ...
  Future<List<Additive>?> _getSideAdditive(String id) async {
    var db = await database;
    var result = await db.query(DBSideAdditive.tableName, where: '${DBSideAdditive.columnSideID} = ?', whereArgs: [id]);
  ...
  Future<NutritionData?> _getNutritionData(String id, NutritionEntity entity) async {
    NutritionDataMixin? dbNutritionData = entity == NutritionEntity.meal ? await _getDBMealNutritionData(id) : await _getDBSideNutritionData(id);
  ...
  Future<List<Canteen>?> getCanteens() async {
    ...
    return Future.value(result.isNotEmpty ? result.map((canteenRow) => DBCanteen.fromMap(canteenRow)).map((e) => Canteen(id: e.canteenID, name: e.name)).toList() : null);
  }
  ...
  Future<void> removeImage(ImageData image) async {
    var db = await database;
    await db.delete(DBImage.tableName, where: '${DBImage.columnImageID} = ?', whereArgs: [image.id]);
  }

  @override
  Future<void> updateCanteen(Canteen canteen) async {
    var db = await database;
    await db.insert(DBCanteen.tableName, DBCanteen(canteen.id, canteen.name).toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> cleanUp() async {
    ...
    List<DBMealPlan> mealPlans =
        (await db.query(DBMealPlan.tableName, where: '${DBMealPlan.columnDate} < ?', whereArgs: [_dateFormat.format(DateTime.now().subtract(const Duration(days: 1)))]))
            .map((e) => DBMealPlan.fromMap(e))
            .toList();

    for (DBMealPlan mealPlan in mealPlans) {
      List<DBMealPlanMeal> mealPlanMeals = (await db.query(DBMealPlanMeal.tableName, where: '${DBMealPlanMeal.columnMealPlanID} = ?', whereArgs: [mealPlan.mealPlanID]))
          .map((e) => DBMealPlanMeal.fromMap(e))
          .toList();
      List<DBMealPlanSide> mealPlanSides = (await db.query(DBMealPlanSide.tableName, where: '${DBMealPlanSide.columnMealPlanID} = ?', whereArgs: [mealPlan.mealPlanID]))
          .map((e) => DBMealPlanSide.fromMap(e))
          .toList();
      for (DBMealPlanSide mealPlanSide in mealPlanSides) {
        if ((await db.query(DBMealPlanSide.tableName, where: '${DBMealPlanSide.columnSideID} = ?', whereArgs: [mealPlanSide.sideID])).length > 1) {
          print("Side ${mealPlanSide.sideID} is used in another meal plan");
          continue;
        }
        db.delete(DBSide.tableName, where: '${DBSide.columnSideID} = ?', whereArgs: [mealPlanSide.sideID]);
        db.delete(DBSideAllergen.tableName, where: '${DBSideAllergen.columnSideID} = ?', whereArgs: [mealPlanSide.sideID]);
        db.delete(DBSideAdditive.tableName, where: '${DBSideAdditive.columnSideID} = ?', whereArgs: [mealPlanSide.sideID]);
        db.delete(DBSideNutritionData.tableName, where: '${NutritionDataMixin.columnEntityID} = ?', whereArgs: [mealPlanSide.sideID]);
        db.delete(DBMealPlanSide.tableName, where: '${DBMealPlanSide.columnSideID} = ?', whereArgs: [mealPlanSide.sideID]);
        print("Deleted side ${mealPlanSide.sideID}");
      }
      for (DBMealPlanMeal mealPlanMeal in mealPlanMeals) {
        if ((await db.query(DBFavorite.tableName, where: '${DBFavorite.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID])).isNotEmpty) {
          print("Meal ${mealPlanMeal.mealID} is a favorite");
          continue;
        }
        if ((await db.query(DBMealPlanMeal.tableName, where: '${DBMealPlanMeal.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID])).length > 1) {
          print("Meal ${mealPlanMeal.mealID} is used in another meal plan");
          continue;
        }
        db.delete(DBMeal.tableName, where: '${DBMeal.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID]);
        db.delete(DBMealAllergen.tableName, where: '${DBMealAllergen.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID]);
        db.delete(DBMealAdditive.tableName, where: '${DBMealAdditive.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID]);
        db.delete(DBMealNutritionData.tableName, where: '${NutritionDataMixin.columnEntityID} = ?', whereArgs: [mealPlanMeal.mealID]);
        db.delete(DBImage.tableName, where: '${DBImage.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID]);
        db.delete(DBMealPlanMeal.tableName, where: '${DBMealPlanMeal.columnMealID} = ?', whereArgs: [mealPlanMeal.mealID]);
        print("Deleted meal ${mealPlanMeal.mealID}");
      }
      db.delete(DBMealPlan.tableName, where: '${DBMealPlan.columnMealPlanID} = ?', whereArgs: [mealPlan.mealPlanID]);
